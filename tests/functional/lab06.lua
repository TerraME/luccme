...
return {
	lab06 = function(unitTest)
		...
		local env_Lab06 = Environment{}
		env_Lab06:add(timer)

		-- ENVIROMMENT EXECUTION
		if Lab06.isCoupled == false then
			local tsave = databaseSave(Lab06)
			env_Lab06:add(tsave)
			env_Lab06:run(Lab06.endTime)
			saveSingleTheme(Lab06, true)
		end

		-- Creating Map for test compare
		local cs2 = CellularSpace{
						file = filePath("test/Lab06_2014.shp", "luccme"),
						zero = "top"
					}
		...
		unitTest:assertSnapshot(mapsResult, "lab06.png")
		...
		projFile = filePath("test/Lab06_2014.shp", "luccme")
		if(projFile:exists()) then
			projFile:delete()
		end
	end,
