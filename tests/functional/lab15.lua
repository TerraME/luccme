...
return {
	lab15 = function(unitTest)
		...
		local env_Lab15 = Environment{}
		env_Lab15:add(timer)

		-- ENVIROMMENT EXECUTION
		if Lab15.isCoupled == false then
			local tsave = databaseSave(Lab15)
			env_Lab15:add(tsave)
		...
		local cs2 = CellularSpace{
						file = filePath("test/Lab15_2004.shp", "luccme"),
						zero = "top"
					}
		...
		unitTest:assertSnapshot(mapsResult, "lab15.png")		
		...
		projFile = filePath("test/Lab15_2004.shp", "luccme")
		if(projFile:exists()) then
			projFile:delete()
		end
	end,
