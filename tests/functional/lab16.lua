...
return {
	lab16 = function(unitTest)
		...
		local env_Lab16 = Environment{}
		env_Lab16:add(timer)

		-- ENVIROMMENT EXECUTION
		if Lab16.isCoupled == false then
			local tsave = databaseSave(Lab16)
			env_Lab16:add(tsave)
		...
		local cs2 = CellularSpace{
						file = filePath("test/Lab16_2004.shp", "luccme"),
						zero = "top"
					}
		...
		unitTest:assertSnapshot(mapsResult, "lab16.png")
		...
		projFile = filePath("test/Lab16_2004.shp", "luccme")
		if(projFile:exists()) then
			projFile:delete()
		end
	end,
