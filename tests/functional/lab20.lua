...
return {
	lab20 = function(unitTest)
		...
		local env_Lab20 = Environment{}
		env_Lab20:add(timer)

		-- ENVIROMMENT EXECUTION
		if Lab20.isCoupled == false then
			local tsave = databaseSave(Lab20)
			env_Lab20:add(tsave)
		...
		local cs2 = CellularSpace{
						file = filePath("test/Lab20_2004.shp", "luccme"),
						zero = "top"
					}
		...
		unitTest:assertSnapshot(mapsResult, "lab20.png")
		...
		projFile = filePath("test/Lab20_2004.shp", "luccme")
		if(projFile:exists()) then
			projFile:delete()
		end
	end,
